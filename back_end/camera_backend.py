...
def api_save():
    ...
    for marker in request_data["markers"]:
        ...
        for image_marker in marker["pictureMarkers"]:
            ...
            image_marker_label = image_marker["label"]
            image_marker_lat = image_marker["lat"]
            image_marker_lng = image_marker["lng"]
            image_marker_picture = image_marker["picture"]

            cur.execute("INSERT INTO image_markers(marker_label, label, lat, lng, picture) VALUES (%s, %s, %s, %s, %s)", (marker_label, image_marker_label, image_marker_lat, image_marker_lng, image_marker_picture))

    for line_segment in request_data["line_segments"]:
        pt1 = line_segment["pt1"]
        pt2 = line_segment["pt2"]
        coord1_lat = line_segment["coord"][0][0]
        coord1_lng = line_segment["coord"][0][1]
        coord2_lat = line_segment["coord"][1][0]
        coord2_lng = line_segment["coord"][1][1]
...
def api_load():
    ...
    for row in cur:
        ...
        temp_map_obj = { 
            "mapImageURL": row[0],
            "mapBounds": [row[1], row[2]],
        }

    map_settings.append(temp_map_obj)
...
def api_update_map():
    ...
    cur.execute("SELECT map_image FROM map_settings LIMIT 1")
    ...
    cur.execute("UPDATE map_settings SET map_image = %s, map_height = %s, map_width = %s LIMIT 1 ", (map_image, map_height, map_width))
